...
void processinput(short snum)
{
    ...
    /* Headless fire-soak (DUKE3D_AUTOPLAY_FIRE=N): the normal autoplay path masks
     * out the Fire bit, so the weapon-fire code (shoot/hitscan/projectile spawn
     * + impact) is never exercised by CI. When requested AND already in a
     * headless autoplay run, force the weapon to fire for up to N *game tics*.
     * Bounding by tics (not render frames) keeps the soak deterministic
            ...
            if( ud.multimode > 1 && ( s->pal != 1 || (s->cstat&32768) ) )
            {
                if(p->frag_ps != snum)
                {
                    ...
                    if( ud.user_name[p->frag_ps][0] != 0)
                    {
                        if(snum == screenpeek)
                        {
                            snprintf(&fta_quotes[115][0],64,"KILLED BY %s",&ud.user_name[p->frag_ps][0]);
                            FTA(115,p);
                        }
                        else
                        {
                            snprintf(&fta_quotes[116][0],64,"KILLED %s",&ud.user_name[snum][0]);
                    ...
                    else
                    {
                        if(snum == screenpeek)
                        {
                            snprintf(&fta_quotes[115][0],64,"KILLED BY PLAYER %ld",1+p->frag_ps);
                            FTA(115,p);
                        }
                        else
                        {
                            snprintf(&fta_quotes[116][0],64,"KILLED PLAYER %ld",1+snum);
                            FTA(116,&ps[p->frag_ps]);
                        }
                    }
                }
